function [xtraj, ttraj, terminate_cond] = test_trajectory(start, stop, map, path, vis, show_plot)
...
%% **************************** FIGURES *****************************
if ~show_plot % If not globally plotting, then setup local plot
    % Environment figure
    if nargin < 5
        vis = true;
    end
    ...
    for qn = 1:nquad
        ...
        if iter == 1
            if show_plot
               ...
               QP{qn} = QuadPlot_mod(qn, x0{qn}, 0.1, 0.04, quadcolors(qn,:), max_iter, h_3d);
            end
